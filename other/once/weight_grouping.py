...
def Quantize_for_mask(x_torch, n, mask_value, default_value):
    ...
    mask            = torch.tensor([-1,-2,-4,-8,-16,-32,-64,-128,-256,-256,-256,-256,-512,-512,-1024,-4096])
    # print("mask :",mask)
...
def shannon_entropy(x_torch):
    ...
    x_count = torch.zeros((65535), dtype=torch.int16)
...
for name, param in tqdm(model.named_parameters()):
    with torch.no_grad():
        ...
        param_save = np.save("/home/kkh/pytorch/data/23.01.31_symbol_INT16/"+ name, Quant_input.cpu().numpy()) # per layer
...
# print("total_param_entropy",shannon_entropy(torch.from_numpy(parameters_index)).cpu())
# param_save = np.save("/home/kkh/pytorch/data/23.01.31_symbol_INT16/total.weight",parameters_index)
...
# write_workbook.save('/home/kkh/pytorch/data/23.01.31_param.xlsx')
result = []

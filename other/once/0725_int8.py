...
# _model = fuse_bn_recursively(models.squeezenet1_0(weights=models.SqueezeNet1_0_Weights.IMAGENET1K_V1)).to('cuda')
...
for name, param in tqdm(model.named_parameters()):
    with torch.no_grad():
        ...
        for i in Data_shape:
            shapes *= i
        if shapes%64 != 0 :
            Quant_input, scale, zero_p = Quant(param,8)#quantize_channel(param,16)
        ...
        Quant_input, scale, zero_p = Quant(param,8)#quantize_channel(param,16)         # input (294*32) INT16
...
# write_workbook.save('/home/kkh/pytorch/data/MobileNet.xlsx')
result = []

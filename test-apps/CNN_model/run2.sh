...
numiter=4
Setiter=0
# bert MobileNet ShuffleNet ViT
coefficient=(7 4 10 1)
...
for file in "$models_directory"/*.py; do
    if [ -f "$file" ]; then
        ...
        if [ -f "$log_file" ]; then
            ...
            if ["$flag" -eq 0]; then
                echo "Executing $file..."
                Setiter=$((numiter * coefficient[numModels]))
                LD_PRELOAD="$shared_library_directory/mem_trace.so" python "$file" $input_data_path --batch_size 4 --iter_size $Setiter --labels $labels_path >> "$log_file"
